...
export function useATMStore() {
  ...
  const login = useCallback((pin: string) => {
    try {
      if (state.isLocked) {
        throw new Error("Account Locked. Contact RupeeATM Support.");
      }
  ...
  const executeTransaction = (type: TransactionType, amount: number, description?: string) => {
    ...
    toast({ 
      ...
      description: `Current balance: ₹${(type === 'deposit' ? state.balance + amount : state.balance - amount).toLocaleString('en-IN')}` 
    });
  ...
  const withdraw = useCallback((amount: number) => {
    ...
    if (amount >= OTP_THRESHOLD) {
      ...
      setOtpRequired(true);
      toast({ title: "OTP Required", description: "Sent a 6-digit OTP to your registered mobile for high-value withdrawal." });
    } else {
